...
namespace PaymentProvider
{
    ...
    String httpPost(const String &url, const String &postData)
    {
        ...
        long millis_elapsed = millis() - millis_start;

        Serial.println("PaymentProvider::httpPost() - Starting HTTP POST to " + url);
        Serial.println("Time elapsed: " + String(millis_elapsed) + " ms");

        millis_elapsed = millis() - millis_start;
        Serial.println("Time elapsed: " + String(millis_elapsed) + " ms");

        Serial.println("PaymentProvider::httpPost() - Doing POST with data: " + postData);
        http.begin(url);
